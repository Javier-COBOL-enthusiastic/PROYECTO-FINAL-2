...
def agregar_equipo(nombre):
    ...
    conexion = obtener_conexion()
    cursor = conexion.cursor()
    cursor.execute("INSERT INTO equipos (nombre_equipo, fecha_creacion) VALUES (%s, %s)", (nombre, fecha_creacion))
    conexion.commit()
    conexion.close()
...
def actualizar_equipo(id_equipo, nuevo_nombre, nueva_fecha_creacion):
    conexion = obtener_conexion()
    cursor = conexion.cursor()
    cursor.execute("UPDATE equipos SET nombre_equipo = %s, fecha_creacion = %s WHERE id_equipo = %s", (nuevo_nombre, nueva_fecha_creacion, id_equipo))
    conexion.commit()
    conexion.close()
